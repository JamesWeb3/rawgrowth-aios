// Smoke local /api/onboarding/chat with anthropic-cli provider.
...
const res = await fetch(URL + "/api/onboarding/chat", {
  ...
  body: JSON.stringify({
    messages: [
      {
        role: "assistant",
        content: "Hi Pedro, ready to get started?",
      },
...
console.log(`\n--- end after ${((Date.now() - t0) / 1000).toFixed(2)}s, ${total.length} bytes ---`);
await browser.close();
