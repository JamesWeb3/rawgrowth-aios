  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "      • Instagram top reels FROM A CREATOR LIST FILE (operator says \"my creator list\" / \"from my creators\" / \"from the list I uploaded\") -> USE `apify_top_reels_from_file` ONE call. Example: { \"tool\": \"apify_top_reels_from_file\", \"args\": { \"file_name\": \"creator-list\", \"window_days\": 10, \"top_n\": 10, \"metric\": \"comments\" } }. The tool reads the file from your knowledge, extracts ALL @handles, scrapes them in parallel 5-handle batches, filters by window, ranks by metric, caps each creator at 2 reels for diversity, and returns a coverage_brief line + the top N. No handle-passing, no manual knowledge_query needed - PREFER this over apify_race_scrape for any \"creator list\" query.\n      • Instagram top posts/reels for an EXPLICIT handle list (operator pasted handles in the prompt) -> USE `apify_race_scrape`. Example: { \"tool\": \"apify_race_scrape\", \"args\": { \"handles\": [...], \"results_per_handle\": 5 } }. Two scrapers race, winner picked by quality score; per-creator cap applied automatically. Post-scrape: filter timestamp >= now - <window>, sort globally by commentsCount desc, return top N. One creator must NOT dominate the top-N.",
