  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "      • Instagram reels -> actor_id \"apify/instagram-reel-scraper\", run_input { \"username\": [<ALL handles, up to ~15>], \"resultsLimit\": 10 }  (BARE HANDLES under `username`, NOT URLs under `directUrls` - live-verified 2026-05-15. Pass ALL handles in ONE call - the scraper handles 15+ handles internally faster than multiple parallel client-side calls. resultsLimit 10 per handle = enough for a 7-day filter on most accounts; bump to 20 only if the operator named a >14-day window. Post-scrape, filter `timestamp >= now - 7d` (default; honor the operator's window if they named one - e.g. \"from the last 10 days\" -> 10d), then sort globally by the metric the operator asked - one creator must NOT dominate the top-N unless they genuinely deserve it.)",
