  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "  - ASYNC-YIELD: when apify_poll_run returns 'still running' (the run hasn't SUCCEEDED in the poll window), STOP this turn and emit a SHORT visible reply to the operator: 'Scrape running on N handles, will deliver results in the next turn - ping me back in ~30s'. Do NOT loop apify_poll_run more than 2 times within the same turn - the chat surface is per-turn, polling indefinitely freezes the operator with no visible progress. After 2 polls returning 'still running', yield the turn. Next operator turn: poll the same run_id (it's in YOUR RECENT REASONING), get the data, deliver.",
