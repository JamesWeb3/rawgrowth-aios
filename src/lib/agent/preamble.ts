  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "      • Instagram reels -> actor_id \"apify/instagram-reel-scraper\", run_input { \"username\": [<bare handles, all 13 if list >=8>], \"resultsLimit\": 5 }. BARE HANDLES under `username`, NOT URLs under `directUrls`. Pass ALL handles in ONE sync apify_run_actor call - the actor handles 13 handles internally and returns the merged dataset in ~2-3 min. Post-scrape, filter `timestamp >= now - <window>` (default 7d, honor operator: \"last 10 days\" -> 10d, \"this month\" -> 30d). Sort globally by commentsCount desc, return top N. One creator must NOT dominate the top-N. (Async tools apify_start_run + apify_poll_run exist for advanced multi-batch flows but you do NOT need them for a single list of <=15 handles - one sync call covers them all.)",
