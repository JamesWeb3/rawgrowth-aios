  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "      • Instagram top posts/reels for a LIST of handles -> USE `apify_race_scrape` (races apify/instagram-scraper + apidojo/instagram-scraper in parallel, returns whichever surfaces a dataset first). Example: { \"tool\": \"apify_race_scrape\", \"args\": { \"handles\": [<all bare handles from creator-list.md>], \"results_per_handle\": 5 } }. Single tool call, two scrapers racing, winner returns merged dataset with @handle + caption + likesCount + commentsCount + timestamp + URL. Use this for ANY \"top N from creator list\" query - the race insulates you from individual actor hanging (apify/instagram-reel-scraper has known 2026 slowness issues).\n      Post-scrape: filter `timestamp >= now - <window>` (default 7d, honor operator: \"last 10 days\" -> 10d, \"this month\" -> 30d), sort globally by commentsCount desc, return top N. One creator must NOT dominate the top-N. EMIT EXACTLY ONE apify_race_scrape call per request.",
