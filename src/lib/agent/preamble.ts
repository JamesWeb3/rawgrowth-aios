  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "      • Instagram reels -> actor_id \"apify/instagram-reel-scraper\", run_input { \"username\": [<up to 8 handles>], \"resultsLimit\": 5 }  (BARE HANDLES under `username`, NOT URLs under `directUrls`. CAP handles at 8 per call + resultsLimit 5 - the run-sync ceiling is 180s, anything bigger hangs. For a longer list, split into 8-handle batches and emit MULTIPLE <command type=\"tool_call\"> blocks in the SAME reply - they run in parallel server-side. Post-scrape, filter `timestamp >= now - 7d` default, or honor the operator's stated window (e.g. \"last 10 days\" -> 10d, \"this month\" -> 30d). Sort globally by the metric the operator asked (commentsCount desc by default). One creator must NOT dominate the top-N unless they genuinely deserve it.)",
        ...
        "  - EXACT-HANDLES: when copying identifiers (Instagram handles, email addresses, usernames, slugs) from a file or memory into a tool call, use the EXACT string verbatim. Do NOT abbreviate, paraphrase, or normalize - 'thejasminearielle' is NOT 'jasmine', 'sundaysolves' is NOT 'sundayboss', 'heyriley.ai' is NOT 'heyriley'. A single missing character returns zero results, and you'll mistakenly report the user's list as 'low engagement' when you simply scraped the wrong account. If you're not sure of the exact spelling, RE-READ the source (knowledge_query the file, scroll the recent reasoning) - do not guess.
