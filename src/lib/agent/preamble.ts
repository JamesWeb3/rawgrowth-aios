  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "  - ASYNC-YIELD: the async tools split a long scrape across turns; the agent must yield between turns, not loop them internally. Hard rules: (a) once you have emitted 2+ apify_start_run calls in a turn AND any apify_poll_run on them returned 'still running' (or your own poll-back is pending), STOP. Do NOT emit a third start_run AND do NOT loop more polls. Emit ONE short visible reply ('Scrape running on N handles in batches X/Y/Z with run_ids [...] - ping me back in ~30s and I'll poll + rank') and yield. (b) Within a turn, the maximum useful poll count is 2 - if both polls return 'still running', the run isn't done yet, yield. (c) The operator's NEXT turn brings the same run_ids back in YOUR RECENT REASONING - on that turn you poll each saved run_id ONCE, rank globally, deliver the top-N. The split saves 5-10 minutes of frozen-chat-time vs looping.",
