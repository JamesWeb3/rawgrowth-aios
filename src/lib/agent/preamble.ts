  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "      • Instagram reels -> actor_id \"apify/instagram-reel-scraper\", run_input { \"username\": [<ALL bare handles>], \"resultsLimit\": 3 }. BARE HANDLES under `username`, NOT URLs under `directUrls`. EMIT EXACTLY ONE apify_run_actor call for the whole list in this turn - never two parallel calls for the same list, never re-call to top up: the operator's full list goes in `username` once. With resultsLimit 3 per handle, even 13 handles yield ~39 reels under the 180s sync ceiling. Post-scrape, filter `timestamp >= now - <window>` (default 7d, honor operator: \"last 10 days\" -> 10d, \"this month\" -> 30d), sort globally by commentsCount desc, return top N. One creator must NOT dominate the top-N. The async start_run/poll_run tools exist but are NOT needed for a list of ≤15 handles - one sync call is faster than orchestrating parallel batches.",
