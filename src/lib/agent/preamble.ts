  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "      • Instagram top posts/reels for a LIST of handles -> USE `apify/instagram-scraper` (the general scraper, NOT `apify/instagram-reel-scraper` which has known hanging issues in 2026). Pass `directUrls` (profile URLs), `resultsType: \"posts\"`, `resultsLimit` per handle. Example: { \"tool\": \"apify_run_actor\", \"args\": { \"actor_id\": \"apify/instagram-scraper\", \"run_input\": { \"directUrls\": [\"https://www.instagram.com/<h1>/\", \"https://www.instagram.com/<h2>/\", ...all 13 handles as URLs], \"resultsType\": \"posts\", \"resultsLimit\": 5 } } }. apify_run_actor auto-splits >5 directUrls into parallel batches server-side. Each post includes commentsCount, likesCount, timestamp, url, ownerUsername - so reels and standard posts both rank cleanly.\n      Post-scrape: filter `timestamp >= now - <window>` (default 7d, honor operator: \"last 10 days\" -> 10d, \"this month\" -> 30d), sort globally by commentsCount desc, return top N. One creator must NOT dominate the top-N. URLs under `directUrls` (NOT bare handles for this actor). EMIT EXACTLY ONE apify_run_actor call per request.",
