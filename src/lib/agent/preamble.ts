  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "      • Instagram reels (>5 handles) -> use the ASYNC pattern: emit apify_start_run for each batch (returns a run_id in ~2s, does NOT wait), then apify_poll_run on each run_id (waits up to 30-60s per poll, retries next turn if still running). Sync apify_run_actor hangs the chat turn on a 13-handle list; async lets you fire all batches at once and collect them as they finish.\n        async start (per 8-handle batch): { \"tool\": \"apify_start_run\", \"args\": { \"actor_id\": \"apify/instagram-reel-scraper\", \"run_input\": { \"username\": [<up to 8 BARE handles>], \"resultsLimit\": 5 } } } - returns { run_id, dataset_id, status }.\n        async poll: { \"tool\": \"apify_poll_run\", \"args\": { \"run_id\": \"<from start_run>\", \"max_wait_seconds\": 30, \"limit\": 50 } } - returns dataset items when done, or \"still running\" if not.\n      • Instagram reels (<=5 handles) -> apify_run_actor sync is fine: { \"tool\": \"apify_run_actor\", \"args\": { \"actor_id\": \"apify/instagram-reel-scraper\", \"run_input\": { \"username\": [<bare handles>], \"resultsLimit\": 5 } } }\n        RANKING: post-scrape, filter `timestamp >= now - 7d` default (honor operator window: \"last 10 days\" -> 10d, \"this month\" -> 30d). Sort globally by commentsCount desc. One creator must NOT dominate the top-N. BARE HANDLES under `username`, NOT URLs under `directUrls`.",
        ...
        "  - ASYNC-YIELD: the async tools (apify_start_run + apify_poll_run) SPLIT a long scrape across turns. Yielding turn = emitting your visible reply to the operator and stopping; the operator's next message resumes you. RULE: once you have 4+ tool cards in the current turn's orchestration timeline (any combination of start_run, poll_run, knowledge_query, composio_use_tool), the NEXT thing you generate MUST be your visible reply, not another tool call. State plainly what you fired (batch A with handles X/Y, run_id <id>) and what next turn will do (poll those run_ids, rank, deliver). The operator's reply restarts you with the same run_ids in YOUR RECENT REASONING. Stalling 10 minutes in one turn with 6+ tool cards and no visible reply is the bug - the operator thinks the agent died. Yield is not a fallback, it's the path. The split saves 5-10 min of frozen chat time.",
