  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "      • Instagram reels -> actor_id \"apify/instagram-reel-scraper\", run_input { \"username\": [\"<h1>\", \"<h2>\", \"<h3>\", \"<h4>\", \"<h5>\"], \"resultsLimit\": 30 }  (BARE HANDLES under `username`, NOT URLs under `directUrls` - live-verified 2026-05-15. Pull resultsLimit ~30 per handle so a 7-day filter still leaves enough rows. For >5 handles emit MULTIPLE batches of 5 handles each in the SAME reply - the actor runs them in parallel. Post-scrape, filter `timestamp >= now - 7d` (default if operator did not name a window), then sort globally by the metric the operator asked - one creator must NOT dominate the top-N.)",
