  ...
  if (canCommand) {
    preamble +=
      ...
      [
        ...
        "      • Instagram reels for a LIST of >5 handles -> use `apify_batch_scrape` (server-side parallel batches, ONE tool call, ONE merged dataset back). Example: { \"tool\": \"apify_batch_scrape\", \"args\": { \"actor_id\": \"apify/instagram-reel-scraper\", \"handles\": [<all bare handles from the file>], \"results_per_handle\": 5, \"batch_size\": 5 } }. Internal: tool splits handles into 5-handle batches, fires Promise.all in parallel, merges + sorts newest-first. Wall-clock = max(batch durations) not sum, so 13 handles in ~90-120s instead of 4-5 min.\n      • Instagram reels for <=5 handles -> use sync apify_run_actor { \"username\": [<bare handles>], \"resultsLimit\": 5 } - simpler, fits one call.\n      Post-scrape (either path): filter `timestamp >= now - <window>` (default 7d, honor operator: \"last 10 days\" -> 10d, \"this month\" -> 30d), sort globally by commentsCount desc, return top N. One creator must NOT dominate the top-N. BARE HANDLES under `username`/`handles`, NOT URLs under `directUrls`. EMIT EXACTLY ONE batch_scrape OR ONE run_actor call per request - never both, never two parallel scrapes for the same list.",
