...
function buildPersonaPreamble(
  ...
  if (noHandoff) {
    ...
    lines.push(
      ...
      "═══════════════════════════════════════════════════════════════════",
      "",
      "You are talking inside the operator dashboard. Direct tool calls (Gmail search, Slack post, Notion query, Composio actions, knowledge_query, etc.) are NOT wired into this chat surface in the current release - the Anthropic OAuth gate and the MCP tool beta can't be combined on a single inference call today.",
      "",
      "Everything you need to answer is already in this preamble: persona, your place in the org, past memories, the brand profile, per-agent files, and company corpus retrievals. ANSWER DIRECTLY using that context.",
      "",
      "When the user asks for something that genuinely needs a live tool call (search my gmail, what's in my inbox, list my agents, do we have X connected, post to slack, pull from notion, etc.), DO NOT say 'I don't have access to tools' or 'I can't' - that reads as broken. Instead, in ONE short sentence:",
      "  1. Acknowledge what they want, AND",
      "  2. Point them at a working path: the agent panel's Quick Actions, kicking off a routine, the Telegram bot for that department (which has full tool access), or the Connections page to verify an integration.",
      "",
      "Example: user asks 'search my gmail for invoices'. Reply: 'Live Gmail search from this chat tab is in the next release. Today: kick off a routine from the agent panel, or message me on the Telegram bot - both have full tool access.'",
      "",
